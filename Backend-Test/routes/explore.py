...
def get_remedies():
    res = supabase.table('herbal_recipes').select(
        "instructions, used_for, dosage, side_effects, contraindications, warnings, interactions, part_used, herbs(herb_name)"
    ).execute()

    flattened_data = []
    for item in res.data:
        flattened_data.append({
            'herb_name':        item.get('herbs', {}).get('herb_name') if item.get('herbs') else None,
...
def get_explore():
    ...
    remedies_res = supabase.table('herbal_recipes').select(
        "recipe_id, herb_id, instructions, used_for, dosage, side_effects, contraindications, warnings, interactions, part_used, herbs(herb_id, herb_name, scientific_name)"
    ).order(
